...
typedef struct Matrix{//estructura para definir la matriz
	...
	int **values;//es el puntero con doble indireccion para crear los espacio a la las filas y las columas
}matriz;
...
int main(void)
{
	...
	n=10;//MENU(n);
		while (n!=-1)
		{
			...
			eventos(pm);
			Libera(pm);
			n=-1;//MENU(n);//vuelve a pedir el menu a menos que se desee finaliar la simulacion, poniendo en n=-1
		}
